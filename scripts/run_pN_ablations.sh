...
RNA_DIR=${RNA_DIR:-datasets/pN_combined}
ATAC_DIR=${ATAC_DIR:-datasets/pN_combined_ATAC}
...
OUTPUT_ROOT=${OUTPUT_ROOT:-output/pN_ablations_$(date +%Y%m%d_%H%M%S)}
mkdir -p "$OUTPUT_ROOT"
...
# Optional: inference-time stress tests
ENABLE_CANDIDATE_SWAP_STRESS_TEST=${ENABLE_CANDIDATE_SWAP_STRESS_TEST:-0}

# ------------------------------
# Optional fast GRN export knobs
# ------------------------------
# These flags only affect the GRN CSV/logit export workload.
# They are especially useful for pN where full export can take many hours.
EXPORT_MAX_CELLS=${EXPORT_MAX_CELLS:-""}             # e.g. 2000 (empty => use all cells)
EXPORT_CELL_SAMPLING=${EXPORT_CELL_SAMPLING:-"random"}  # first|random
EXPORT_CELL_SEED=${EXPORT_CELL_SEED:-42}
EXPORT_TF_CHUNK_SIZE=${EXPORT_TF_CHUNK_SIZE:-""}     # e.g. 64
EXPORT_TARGET_CHUNK_SIZE=${EXPORT_TARGET_CHUNK_SIZE:-""} # e.g. 2048
EXPORT_REUSE_KNN_INDICES=${EXPORT_REUSE_KNN_INDICES:-1}  # 1 => pass --reuse_knn_indices

EXPORT_TUNING_ARGS=()
if [[ -n "${EXPORT_MAX_CELLS}" ]]; then
  EXPORT_TUNING_ARGS+=(--export_max_cells "${EXPORT_MAX_CELLS}")
  EXPORT_TUNING_ARGS+=(--export_cell_sampling "${EXPORT_CELL_SAMPLING}")
  EXPORT_TUNING_ARGS+=(--export_cell_seed "${EXPORT_CELL_SEED}")
fi
if [[ -n "${EXPORT_TF_CHUNK_SIZE}" ]]; then
  EXPORT_TUNING_ARGS+=(--export_tf_chunk_size "${EXPORT_TF_CHUNK_SIZE}")
fi
if [[ -n "${EXPORT_TARGET_CHUNK_SIZE}" ]]; then
  EXPORT_TUNING_ARGS+=(--export_target_chunk_size "${EXPORT_TARGET_CHUNK_SIZE}")
fi
if [[ "${EXPORT_REUSE_KNN_INDICES}" != "0" ]]; then
  EXPORT_TUNING_ARGS+=(--reuse_knn_indices)
fi
...
# Also export baseline nnPU head under ablation folder for easy comparison
...
for K in $TOPK_STAGE1_LIST; do
  ...
  if [[ ! -f "$A4_K_STAGE2_OUT/grn_head.pth" ]]; then
    ...
    if [[ ! -f "$A4_K_STAGE2_OUT/grn.csv" || ! -f "$A4_K_STAGE2_OUT/grn_logits.pt" ]]; then
      echo "[ablation4][stage2][topk=$K] Missing export(s); exporting from existing head -> $A4_K_STAGE2_OUT"
      python3 training/finetune_epiaware_grn.py \
        --rna_path "$RNA_DIR/rna.npy" \
        --atac_path "$ATAC_DIR/atac.npy" \
        --candidate_file "$CANDIDATE_JSON" \
        ...
        --output_dir "$A4_K_STAGE2_OUT" \
        --eval_only \
        --head_checkpoint "$A4_K_STAGE2_OUT/grn_head.pth" \
        --epochs 0 \
        "${FAIR_STAGE2_ARGS[@]}" \
        "${EXPORT_TUNING_ARGS[@]}" \
        --export_logit_path "$A4_K_STAGE2_OUT/grn_logits.pt" \
