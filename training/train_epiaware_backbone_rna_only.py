...
from epiaware.models import GeneOnlyPretrainingModel, GeneOnlyTransformer, MaskedNegativeBinomialHead


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Stage 1 (RNA-only) pretraining for EpiAwareNet ablation (gene self-attention only)"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="output/epiaware_backbone_rna_only",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Speeds up runs for large gene counts (approximation)."
        ),
    )
    parser.add_argument(
...
def train() -> None:
    ...
    try:
        ...
        model = GeneOnlyPretrainingModel(backbone, recon_head).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            with tqdm(train_loader, desc=f"Epoch {epoch}/{args.epochs}") as progress:
                for batch in progress:
                    ...
                    micro_batches = [
                        (rna_micro, mask_micro)
                        for rna_micro, mask_micro in zip(rna_chunks, mask_chunks)
                    ...
                    step_loss_value = 0.0
                    for rna_micro, mask_micro in micro_batches:
                        with autocast_context():
                            outputs = model.compute_loss(rna_micro, mask_micro)
            ...
            train_epoch_loss = epoch_total_loss / epoch_steps if epoch_steps > 0 else None
            ...
            if val_loader is not None:
                ...
                val_mask_ratio = args.val_mask_ratio if args.val_mask_ratio is not None else args.mask_ratio
                with torch.no_grad():
                    for batch in val_loader:
                        rna = batch["rna"].to(device)
                        val_mask = make_mask(rna.size(0), rna.size(1), val_mask_ratio, device)
                        with autocast_context():
                            outputs = model.compute_loss(rna, val_mask)
                        ...
                        if zero_mask.any():
                            ...
                            zero_labels.append(
                                (rna.eq(0).masked_select(zero_mask).detach().cpu().numpy())
                            )
        ...
        final_path = output_dir / "epiaware_backbone_rna_only.pth"
        torch.save(model.state_dict(), final_path)
        print(f"[done] Saved pretrained RNA-only backbone to {final_path}")
        if wandb_run is not None:
